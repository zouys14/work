...
int main(){
	...
	page_addr.g.blk = blk;
	page_addr.g.pth = 1;
	if (ssd_group_error_handle(group,group->ssd_list[4],page_addr,ret) == 0)
		printf("error handle failed in %s\n",nvm_dev_get_name(group->ssd_list[4]));
	else
		printf("error handle sucessful in %s\n",nvm_dev_get_name(group->ssd_list[4]));
